 ...
 *  this routine restricts gaps to their minimum length, and partitions
 *  sequences in between continous segments equally */
int clean_block(bloc,nbloc,window)
struct seqdat *bloc;
int nbloc;
int window;
{
	...
	for(i=0; i<bloclen; ++i) {
	  keep[i]=1;
	  for(j=0; j<nbloc; ++j) if(!skip[j]) keep[i]*=(bloc[j+1].seq[i+1]!=' ');
	}
