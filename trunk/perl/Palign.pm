# Rob's Perl alignment/sequence reading/modification/etc. routines
...
#
# Change May 2001
# New field $align->{idsuffix} added.  If defined (for write_afasta) it adds this to
#  the names
...
@EXPORT = qw(read_align get_msf write_msf get_clustal write_clustal get_block write_block get_afasta write_afasta 
	    get_apir write_apir get_pfam get_hmmalign merge_align merge_align_gs merge_align_al do_clustalw_align extract_align get_phylip write_phylip write_intall
	    read_exchange_matrix  get_print_string show_diff prettify write_one_fasta get_swp get_ranges_prop);
...
#
   ...
   for($i=0; $i<=$#data; ++$i) {
	...
	elsif(($_ =~ /^ID  /) || ($_ =~ /^CC  /) || ($_ =~ /^AC  /) || ($_ =~ /^SE  /)) { $votes{"s"}++; $votes{"z"}++; }
	elsif(($_ =~ /^DT  /) || ($_ =~ /^GN  /) || ($_ =~ /^OC  /) || ($_ =~ /^RA  /)) { $votes{"z"}++; }
...
#  anything else will be left as is (e.g. "." characters)
   ...
   for($n=0; $n<=$#data; ++$n) {
      $_ = $data[$n];
      chop;
      if((!/^CLUSTAL/) && (length($_)>1) && (/[A-Za-z0-9\-]/)) {
#        print "Before $_\n";
        s/^ *//;
#        print "After $_\n";
	($label,$seq) = (split(/ +/));
...
#        print "Seqdat goes from |$seqdat| to ";
	$seqdat =~ s/[-\.]/ /g;
#        print "|$seqdat|\n";
...
sub write_afasta {
	...
	for($i=0; $i<$align->{nseq}; ++$i) {
	   $id = $align->{list}[$i];
           if(defined($align->{idsuffix})) {
               $id2 = $id . $align->{idsuffix};
           } else { 
               $id2 = $id;
           }
           if($separate==1) { 
		...
		if($nogap==1) { $seq =~ s/ //g; }
     		write_one_fasta($filename,$id2,$align->{ids}{$id}{prebumpf},$seq);
           } else {
		print OUT ">",$id2," ";
		if(defined($align->{ids}{$id}{prebumpf})) { print OUT $align->{ids}{$id}{prebumpf}," "; }
		if(defined($align->{ids}{$id}{postbumpf})) { print OUT $align->{ids}{$id}{postbumpf}," "; }
...
#                                print "1Merged alignment gets ",$align1->{p}[$counter1]{post},"\n";
			}
			$align3->{alen}++; 
                        $counter1++;
#			print "\n";
...
#                                print "2Merged alignment gets ",$align2->{p}[$counter2]{post},"\n";
			}
			$align3->{alen}++; $counter2++;
#			print "\n";
		}
		last if(($counter1>=$align1->{alen}) && ($counter2>=$align2->{alen}));
		if(uc(substr($align1->{ids}{$linker}{seq},$counter1,1)) ne uc(substr($align2->{ids}{$linker}{seq},$counter2,1))) {
			...
			print STDERR "\n";
			print "Error: linking with sequence ",$linker," mismatch found at alignment positions $counter1 and $counter2: ",
				substr($align1->{ids}{$linker}{seq},$counter1,1),
				" versus ",
				substr($align2->{ids}{$linker}{seq},$counter2,1),
				"\nSeq1 = $align1->{ids}{$linker}{seq}\n",
...
#			}
			if(defined($align1->{p}[$counter1]{post})) {
				$align3->{p}[$align3->{alen}]{post} .= $align1->{p}[$counter1]{post};
#                                print "3aMerged alignment gets ",$align1->{p}[$counter1]{post},"\n";
			}
			if(defined($align2->{p}[$counter2]{post})) {
				$align3->{p}[$align3->{alen}]{post} .= $align2->{p}[$counter2]{post};
#                                print "3bMerged alignment gets ",$align2->{p}[$counter2]{post},"\n";
			}
...
sub merge_align_gs { 
        # arguments: align1 align2 (linker)
        # Difference is that first alignment is treated as gold standard 
        #  (and rather stupidly) no gaps are introduced into it
...
#		print "Here2\n";
		last if(($counter1>=$align1->{alen}) && ($counter2>=$align2->{alen}));
		while(substr($align2->{ids}{$linker}{seq},$counter2,1) eq " ") { 
                       # if blank, copy the rest and put blanks in the other alignment
                       # Commented out here
			last if(($counter1>=$align1->{alen}) && ($counter2>=$align2->{alen}));
#			printf("2 %4d %4d %4d %4d - \n",$pointer1,$counter1,$pointer2,$counter2);
#			foreach $id1 (keys %{$align1->{ids}}) {
...
sub merge_align_al { # arguments: align1 align2 (linker)


	# Merges alignments
	# Difference is that here the linker sequences are re-aligned 
	...
	if($debug==1) { 
        ...
        # First create a sequence reference for alignment
        $seqs->{nseq} = 2;
        $seqs->{list} = ();
        $id1 = $linker . "-one";
        $id2 = $linker . "-two";
        push(@{$seqs->{list}},$id1);
        push(@{$seqs->{list}},$id2);
        $seqs->{ids}{$id1}{seq} = $align1->{ids}{$linker}{seq};
        $seqs->{ids}{$id1}{seq} =~ s/[ \-\.]//g;
        $seqs->{ids}{$id2}{seq} = $align2->{ids}{$linker}{seq};
        $seqs->{ids}{$id2}{seq} =~ s/[ \-\.]//g;
        $aligntemp = do_clustalw_align($seqs);

        # We must then add the linkers to each alignment
        push(@{$align1->{list}},$id1);
        $align1->{nseq}++;
        $align1->{ids}{$id1}{seq} = $align1->{ids}{$linker}{seq};
        $align1->{ids}{$id1}{prebumpf} = $align1->{ids}{$linker}{prebumpf};
        $align1->{ids}{$id1}{postbumpf} = $align1->{ids}{$linker}{postbumpf};

        push(@{$align2->{list}},$id2);
        $align2->{nseq}++;
        $align2->{ids}{$id2}{seq} = $align2->{ids}{$linker}{seq};
        $align2->{ids}{$id2}{prebumpf} = $align2->{ids}{$linker}{prebumpf};
        $align2->{ids}{$id2}{postbumpf} = $align2->{ids}{$linker}{postbumpf};

        # Now just run merge_align twice
        $align3 = merge_align($align1,$aligntemp,$id1);
#        print "align3 length $align3->{alen}, size $align3->{nseq}\n";
        $align4 = merge_align($align3,$align2,$id2);
#        print "align4 length $align4->{alen}, size $align4->{nseq}\n";

#	return $align4; 

        $align5 = extract_align($align4,$id1,1,1);
	return $align5;
}
...
sub do_clustalw_align {
	...
	$clustalw = "/russell/rg1/lbin/clustalw";
...
sub extract_align {
	...
	if($invert == 1 ) { # This is like a grep -v 
		...
		$ignore = 0;
		for($j=0; $j<=$#extract; ++$j) {
		   if((($exact == 0) && ($id =~ /$extract[$j]/)) || ($id eq $extract[$j])) {
			$ignore = 1;
		   }
		}
		if($ignore==0) {
...
#	print "Sequences to use are $set\n";
	for($i=0; $i<$align->{nseq}; ++$i) {
		...
		if($set =~ /$id/) {
			$use{$id} = 1;
#			print "Using $id\n";
			if(($id!~ /_dssp/) && ($id !~ /Sec_/) && ($id !~ /^space/) && ($id !~ /notseq/)) { 
				if($start==-1) { $start = $i; }
				$end = $i;
				$nseqs++;
                                $is_seq{$id}=1;
...
#		print "Pos ",$i," cys: ",$n_cysteine," small: ",$n_small," hydrophobic: ",$n_hydrophobic," polar ",$n_polar," min: ",$min_aa,"\n";
		...
		if($n_cysteine>=$min_aa) {
			$r = get_ranges_prop($align,$i,$start,$end,"C",$is_seq);
			for($j=0; $j<=$#{$r->{range}}; ++$j) {
	...
	for($i=0; $i<$align->{alen}; ++$i) {
		...
		for($j=0; $j<$align->{nseq}; ++$j) {
			...
			if(($aa =~ /[A-Za-z]/) && ($use{$id}==1) && 
				...
				if($polar =~ /$aa/) { $n_polar++; }
				if($aa eq $first_aa) { $n_id++; }
                                else { print OUT "BOX_REGION ",$i+1," ",$j+1," ",$i+1," ",$j+1,"\n"; }
			}
		}
	        if($n_id < $align->{nseq}) {
#		   print "Pos ",$i," cys: ",$n_cysteine," small: ",$n_small," hydrophobic: ",$n_hydrophobic," polar ",$n_polar," min: ",$min_aa,"\n";
...
#  a set of ranges that match what is taken to be a conserved property
   ...
   for($i=$start; $i<=$end; ++$i) {
	 ...
	 $r->{range}[$nr]{st} = $i+1;
         while(($set =~ /$aa/) && ($i<=$end)) {
            $id = $align->{list}[$i];
#	    print "Use is $is_seq{$id} for $id\n";
            $aa = substr($align->{ids}{$id}{seq},$pos,1);
	    $i++;
            last if($is_seq{$id}==0);
	    if($set =~ /$aa/) { $r->{range}[$nr]{en} = $i }
         }
	 $i--;
	 $nr++;
      }
...
#	   print "Read id $id\n";
           ...
           $align->{list}[$name_count] = $id;
           $align->{ids}{$id}{seq} = "";
           $align->{ids}{$id}{prebumpf} = "";
	   $name_count++;
       } elsif(/^DE /) {
           s/DE *//;
           $align->{ids}{$id}{prebumpf} .= $_;
       } elsif(/^SQ /) {
           $in_seq = 1;
       }
   }
