...
int treewise(domain,ndomain,parms)
...
struct parameters *parms;
{
	...
	fprintf(parms[0].LOG,"tree read in from %s and %s:\n",parms[0].ordfile,parms[0].treefile);
...
*/
	   if(parms[0].NPASS==2) {
		...
		printf(") Sc %5.2f RMS %6.2f Len %3d nfit %3d \n", score,rms,length,nfit);
		if(score<2.0) {
		...
		printf(" See file %s.%d for the alignment and transformations\n",parms[0].transprefix,i+1);
	     ...
	     for(j=0; j<cl[i].b.number; ++j) {
	       k=cl[i].b.member[j];
	       update(domain[k].r,domain[k].v,domain[k].R,domain[k].V);
	     }
