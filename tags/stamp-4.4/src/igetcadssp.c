...
/* SMJS Changed to be like Robs version */
		 ...
		 (*ncoord)++;
		 if((*ncoord)>MAXats) {
		    fprintf(stderr,"error: number of coordinates read surpasses memory limit [igetcadssp]\n");
		    return -1;
		 }
		 aa[(*ncoord)]=' ';
	      } /* end of if(begin... */
	...
	free(buff);
	if(!begin) {
	   fprintf(stderr,"error: begin of sequence not found in PDB file [igetcadssp]\n");
	   (*ncoord)=0;
	   free(ccoord);
	   return -1;
	} else {
