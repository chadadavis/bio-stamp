/******************************************************************************
 ...
 E-mail: russell@embl-heidelbergde         E-mail geoff@compbiodundee.ac.uk
 WWW: http://wwwrussellemb-heidelberg.de  WWW: http://www.compbio.dundee.ac.uk

   The WORK is Copyright (1997,1998,1999,2010) Robert B Russell & Geoffrey J Barton
 ...
 * returns 0 if all is well, -1 if an error occurs, 1 if EOF occurs */
{
    ...
    if(DSSP) {
	sprintf(dirfile,"%s/dsspdirectories",env);
    } else {
	sprintf(dirfile,"%s/pdbdirectories",env);
    }
